...
function PortfolioRiskDashboardContent() {
  ...
  useEffect(() => {
    console.log('[PortfolioRisk] Component mounted, loading portfolios and holdings...');
    loadPortfolios();
    ...
    const checkApi = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.warn('[PortfolioRisk] API check returned:', response.status, '- Backend may not be running');
        }
      } catch (err) {
        // Don't set error - just log it, as backend might not be running yet
        console.warn('[PortfolioRisk] API connectivity check failed (backend may not be running):', err);
      }
    };
    checkApi();
  }, []);
  ...
  ), [timeframe, loading, error, analysisStatus, recommendations, holdings, selectedHolding, handleAnalyzeHolding, benchmarkComparison, loadingBenchmark, domain]);
  ...
  const handleProfileFormSubmit = async (formData: InvestmentPhilosophyFormData) => {
    try {
      ...
      console.error('Error saving investment philosophy profile:', error);
      throw error;
    }
  };
